...
class ServerlessAppWorkflow:
    def __init__(self, G, TCO, delayType='SFN', platform='AWS', pricing_model=None, PGS=None, PPI=None):
        print('TCO', TCO)
        if ('Start' in G.nodes and 'End' in G.nodes):
            ...
            G.nodes['Start']['mem'] = 0
            G.nodes['End']['mem'] = 0
            G.nodes['Start']['public_perf_profile'] = {0: 0}
            G.nodes['Start']['private_perf_profile'] = {0: 0}

            G.nodes['End']['public_perf_profile'] = {0: 0}
            G.nodes['End']['private_perf_profile'] = {0: 0}
        ...
        # Get Responce Time of each node And Edge
    ...
    # Calculate the sum of RT of each path in a given graph. -> this will Consider Ne aswell
    ...
    def getTP(self, G, paths):
        ...
        if (type(paths[0]) == list):
            ...
            for path in map(nx.utils.pairwise, paths):
                res.append(np.prod(list(map(lambda edge: G.get_edge_data(
                    edge[0], edge[1])['weight'], list(path)))))
            return res
        else:
    ...
    def get_avg_NE(self, G, startPoint):
        ...
        while [item for item in nx.simple_cycles(G)] != []:
            ...
            self_loops = [(item[0], item[2]['weight'], ne[item[0]])
            ...
            for item in cycles:
                if (G.has_edge(item[-1], item[0]) and (item[0], item[-1]) not in cycles_dict.keys()):
                    cycles_dict[(item[0], item[-1])
                                ] = G.get_edge_data(item[-1], item[0])['weight']
            for key in cycles_dict:
                ...
                out_edges = {item: G.get_edge_data(item[0], item[1])[
                    'weight'] for item in G.out_edges(key[1])}
                tp_list = [out_edges[item] for item in out_edges]
                if (round(np.sum(tp_list), 4) > 1.0):
                    raise Exception(f'Invalid Loop: Node:{key[1]}')
                for node in nodes_in_cycle:
                    G.nodes[node]['ne'] = G.nodes[node]['ne'] / \
                        (1.0 - cycles_dict[key])
    ...
    def get_avg_cost(self):
        # print('----------------inside get_avg_cost function-------------------')
        num_exe = [item for item in self.ne.values()]
        # print('----------------num_exe-------------------', num_exe)

        self.mem = nx.get_node_attributes(self.workflowG, 'mem')
        self.memtype = nx.get_node_attributes(self.workflowG, 'config_type')
        # print("memtype in get_avg_cost", self.memtype)

        # if len(self.memtype) > 1:
        #     print('------------------------self.memtype---------------------------', self.memtype)
        # else:
        #     print('------------------------self.memtype is empty---------------------------', self.memtype)
        ...
        if self.platform == 'AWS':
            mem_cost = [self.pricing_model[item]
                        for item in self.mem.values()]  # Rounding PGS for AWS billing
        ...
        # print('billed_duration', billed_duration)
        avg_cost = np.multiply(
            (np.multiply(mem_cost, billed_duration) + self.ppr), num_exe)
        # print('-----------------------------avg_cost----------------------------------', avg_cost)
        # print('avg_cost', avg_cost)
        # private_fix = 
        return_cost_value = 0
        if len(self.memtype) > 1:
            for i in range(1, len(self.memtype)-1):
                if self.memtype[i] == 'public':
                    return_cost_value = return_cost_value + avg_cost[i] * 1000000
                    # print('public count')
                else:
                    # print("**********************PRIVATE GET_AVG_COST*********************************")
                    # print(self.mem[i], type(self.mem[i]))
                    return_cost_value = return_cost_value + (self.TCO * 0.75) / 6
                    # print('add static', return_cost_value)
                    return_cost_value = return_cost_value + ((self.mem[i])/10000) * (self.TCO * 0.25)
                    # print('add variable', return_cost_value)

        # print('-------------------------return_cost_value-----------------------------', return_cost_value)
        return return_cost_value
        # return np.sum(avg_cost) * 1000000

    def get_RTTP_for_paths_with_B_node(self, paths_with_B_node):
        RTTP = []
        for path in paths_with_B_node:
            B_nodes = [node for node in path if type(
                node) == str and node[0] == 'B']
            B_node_rt_dict = {bnode: self.DAGrt.get(
                bnode) for bnode in B_nodes}
            ...
            if (len(B_nodes) > 1):
                rt_list = [self.simpleDAG.nodes[Bnode]['rt_list']
                           for Bnode in B_nodes]
                tp_list = [self.simpleDAG.nodes[Bnode]['tp_list']
                           for Bnode in B_nodes]
                rt_list = [np.sum(item)
                           for item in itertools.product(*rt_list)]
                tp_list = [np.prod(item)
                           for item in list(itertools.product(*tp_list))]
            else:
    ...
    def process_RTTP(self, RTTP, rt_tp1_path):
        ...
        refined_rt = [(np.max([tup[0] for tup in item]))
                      for item in TPPT_product]
        refined_tp = [(round(np.product([tup[1] for tup in item]), 4))
                      for item in TPPT_product]
    ...
    def drawGraph(self, G):
        ...
        nx.draw_networkx_labels(G, pos_higher_offset,
                                labels=labels, label_pos=0)
        plt.show()
    ...
    def get_parallel_paths_p1(self, G):
        ...
        for u in G.nodes():
            for v in G.nodes():
                ...
                paths_tp1_filter = filter(lambda p: G.get_edge_data(
                    p[0], p[1])['weight'] == 1, filtered_path)
                paths_tp1 = []
                try:
    ...
    def isSimple(self):
        try:
            ...
            pass
        B_node_num = len([None for item in self.simpleDAG.nodes if type(
            item) == 'str' and item[0] == 'B'])
        if B_node_num != 0:
            return False
        TP_sum = 0
        paths = nx.all_simple_paths(
            self.simpleDAG, self.startPoint, self.endPoint)
        for path in map(nx.utils.pairwise, paths):
            TP_sum += (
    ...
    def merge_parallel_paths(self, parallel_paths):
        ...
        self.simpleDAG.add_weighted_edges_from(
            [(max_rt_path[0], node_name, 1)])
        self.simpleDAG.add_weighted_edges_from(
            [(node_name, max_rt_path[-1], 1)])
    ...
    def process_self_loops(self):
        processed = False
        self_loops = [item for item in nx.selfloop_edges(
            self.simpleDAG, data=True)]
        ...
        for aSelfLoop in self_loops:
            ...
            if (round(np.sum(tp_list), 8) != 1.0):
                raise Exception(f'Invalid Self-loop: Node:{aSelfLoop[0]}')
            new_rt = aSelfLoop[2] + aSelfLoop[1] * aSelfLoop[2] + \
                0.5 * (aSelfLoop[1] ** 2) * aSelfLoop[2]
            ...
            tp_denominator = 1.0 - aSelfLoop[1]
            for edge in out_edges:
                self.simpleDAG[edge[0]][edge[1]
                                        ]['weight'] = out_edges[edge] / tp_denominator
        self.updateRT()
        return processed
        ...
        for pp in parallel_paths:
            ...
            if (len(paths_with_B_node) != 0):
                ...
                for PNode in refined_RTTP:
                    ...
                    self.simpleDAG.add_weighted_edges_from(
                        [(pp[0][0], node_name, PNode[1]), (node_name, pp[0][-1], 1)])
                    self.nodeDelay[node_name] = 0
    ...
    def simplify_branches(self):
        ...
        for pp in parallel_paths:
            out_degree_pr = sum(
                map(lambda p: self.simpleDAG.get_edge_data(p[0], p[1])['weight'], pp))
            if (out_degree_pr == 1):
                ...
                self.simpleDAG.add_weighted_edges_from(
                    [(pp[0][0], node_name, 1), (node_name, pp[0][-1], 1)])
                self.nodeDelay[node_name] = 0
                self.edgeDelay[(pp[0][0], node_name)] = 0
                self.edgeDelay[(node_name, pp[0][-1])] = 0
                ...
                processed = True
                continue
            path_tp1_filter = filter(
                lambda p: self.simpleDAG.get_edge_data(p[0], p[1])['weight'] == 1, pp)
            try:
            ...
            paths_gte_path_tp1_rt_filter = filter(
                lambda x: paths_sum_rt[x] >= path_tp1_rt, range(0, len(pp)))
            ...
            if (paths_tpn1_prob_sum > 1):
                raise Exception(
                    'The sum of probabilities is greater than 1. Paths:{}'.format(pp))
            self.simpleDAG[path_tp1[0]][path_tp1[1]
                                        ]['weight'] = 1.0 - paths_tpn1_prob_sum
            processed = True
            paths_lt_path_tp1_rt_index = list(
                set(range(0, len(pp))) - set(paths_gte_path_tp1_rt_index))
    ...
    def simplify_loops(self):
                ...
                if (round(cycles_dict[key]['tp_sum'], 4) != 1):
                    ...
                    for node_in_cycle in nodes_in_cycle:
                        if node_in_cycle == key[0] or node_in_cycle == key[1]:
                            continue
                        out_nodes = set(
                            [edge[1] for edge in self.simpleDAG.out_edges(node_in_cycle)])
                        if out_nodes & set(nodes_in_cycle) != out_nodes:
                            jump_nodes = list(
                                out_nodes - (out_nodes & set(nodes_in_cycle)))
                            for jumpnode in jump_nodes:
                                out_edges = {item: self.simpleDAG.get_edge_data(item[0], item[1])['weight'] for item in
                                             self.simpleDAG.out_edges(node_in_cycle)}
                                tp_list = [out_edges[item]
                                           for item in out_edges]
                                if (round(np.sum(tp_list), 4) > 1.0):
                                    raise Exception(
                                        f'Invalid Node: Node:{key[1]}')
                                tp_denominator = 1.0 - \
                                    self.simpleDAG.get_edge_data(
                                        node_in_cycle, jumpnode)['weight']
                                self.simpleDAG.remove_edge(
                                    node_in_cycle, jumpnode)
        ...
        for key in cycles_dict:
            ...
            new_rt = self.DAGrt[key[1]] / (1 - loop_tp) + \
                loop_tp * cycles_dict[key] / (1 - loop_tp)**2
