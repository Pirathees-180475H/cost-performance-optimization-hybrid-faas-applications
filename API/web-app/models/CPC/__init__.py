...
class PerfOpt:
    ...
    def Cost_Performance_ConstraintModel(self, rt_constraint, cost_constraint, cloud_type):
        # print()
        # print()
        # print()
        # print()
        cost_constraint = cost_constraint - 0.3
        print('-----------INITIATE CPO MODEL--------------------')
        ...
        if cloud_type == 'private':
            self.update_App_workflow_mem_rt(self.App, self.private_maximal_mem_configuration, "private", True)
            current_avg_rt = self.private_minimal_avg_rt
            current_avg_cost = self.private_maximal_cost
        ...
        # current_avg_cost = self.public_maximal_cost
        # print("current_avg_cost", current_avg_cost)
        ...
        while (round(performance_surplus, 4) >= 0 and round(cost_surplus, 4) >= 0):
            ...
            for node in critical_path: #get nodes in a criticle path -> process single node of criticle path
                ...
                if cloud_type == 'public' or cloud_type == 'hybrid':
                    for mem in self.App.workflowG.nodes[node]['public_available_mem']:
                        ...
                        cost_reduction = current_avg_cost - temp_avg_cost #Reduction in cost
                        ...
                        if (increased_rt < performance_surplus and cost_reduction < cost_surplus):          ##### cost reduce
                ...
                if cloud_type == 'private' or cloud_type == 'hybrid':
                    for mem in self.App.workflowG.nodes[node]['private_available_mem']:
                        ...
                        if (increased_rt < performance_surplus and cost_reduction < cost_surplus):
            ...
            current_avg_cost = current_avg_cost - max_cost_reduction
        ...
        response['cost_const'] = cost_constraint
        response['Achived_rt']=round(current_avg_rt, 2)
        response['Achived_cost']=round(current_avg_cost, 2)
