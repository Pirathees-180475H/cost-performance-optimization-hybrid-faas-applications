...
class PerfOpt:
    ...
    def get_optimization_boundary(self):
       ...
       # self.drawGraph(self.App.workflowG)
        ...
        #         maximal_avg_rt))
        return(private_minimal_cost, private_maximal_cost, public_minimal_cost, public_maximal_cost)
    ...
    def CostConstraintModel(self, budget, cloud_type):
        # print()
        # print()
        # print()
        # print()
        ...
        # print('-----------INITIATE COST CONST MODEL--------------------')
        self.update_available_mem_list()  # Only for BCR - False

        if cloud_type == 'public':
            iterations_count_limit = 100
        elif cloud_type == 'private':
            iterations_count_limit = 120
        else:
            iterations_count_limit = 120

        self.update_App_workflow_mem_rt(
            self.App, self.private_minimal_mem_configuration, 'private', True)
        cost = self.private_minimal_cost

        if cloud_type == 'public':
            self.update_App_workflow_mem_rt(
                self.App, self.public_minimal_mem_configuration, "public", True)
            current_cost = self.public_minimal_cost
            current_avg_rt = self.public_maximal_avg_rt
        ...
        cost_surplus = budget - current_cost
        # print("cost", cost)
        ...
        while (round(cost_surplus, 4) >= 0):
            ...
            for node in critical_path:
                ...
                if cloud_type == 'private' or cloud_type == 'hybrid':
                    for mem in reversed(self.App.workflowG.nodes[node]['private_available_mem']):
                        ...
                        # print('--------==============----increased_cost------=================--------', increased_cost, 'surplus: ', cost_surplus)
                        if (increased_cost < cost_surplus):
                            ...
                            if iterations_count > iterations_count_limit:
                                    if increased_cost > 0:
                                        # print('come into 4th if..................................................................')
                                        avg_rt_reduction_of_each_mem_config[str(
                                            mem) + ' private'] = (rt_reduction, increased_cost)
                            else:
                                    avg_rt_reduction_of_each_mem_config[str(
                                        mem) + ' private'] = (rt_reduction, increased_cost)

                                # # print('--------++++++++++++----avg_rt_reduction_of_each_mem_config ADDED------++++++++++++++++++++++++--------', mem, increased_cost, rt_reduction)
                    self.update_App_workflow_mem_rt(self.App, {node: mem_backup[node]}, mem_backupType[node], True)
                    ...
                    for mem in reversed(self.App.workflowG.nodes[node]['public_available_mem']):
                        ...
                        if (increased_cost < cost_surplus):
                            ...
                            if iterations_count > iterations_count_limit:
                                    if increased_cost > 0:
                                        avg_rt_reduction_of_each_mem_config[str(
                                            mem) + ' public'] = (rt_reduction, increased_cost)
                            else:
                                    avg_rt_reduction_of_each_mem_config[str(
                                        mem) + ' public'] = (rt_reduction, increased_cost)
                ...
                if (len(avg_rt_reduction_of_each_mem_config) != 0):
                    ...
                    max_rt_reduction = np.max(
                        [item[0] for item in avg_rt_reduction_of_each_mem_config.values()])
                    ...
                    max_avg_rt_reduction_of_each_node[node] = (
                        reversed_dict[(
                            max_rt_reduction, min_increased_cost_under_MAX_rt_reduction)],
                        max_rt_reduction,
            ...
            max_RT_reduction = np.max(
                [item[1] for item in max_avg_rt_reduction_of_each_node.values()])
            ...
            min_increased_cost_under_MAX_rt_reduction = max_avg_rt_reduction_of_each_node[
                target_node][2]
            # print('++++++++++++++++++min_increased_cost_under_MAX_rt_reduction++++++++++++++++++',min_increased_cost_under_MAX_rt_reduction)
            ...
            current_e2ert_cost_BCR = max_rt_reduction / \
                min_increased_cost_under_MAX_rt_reduction
